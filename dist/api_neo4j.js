...
class neo4j {
    selectionTousCoursNeo4j() {
        return new Promise(function (resolve, reject) {
            ...
            session.run('MATCH (cours:dbjaap_Cours) RETURN cours;')
    ...
    selectionCoursCommenceParNeo4j(data) {
        return new Promise(function (resolve, reject) {
            let driver = neo.driver('bolt://' + host + ':7687', neo.auth.basic(username, passwd));
            let session = driver.session();
            session.run('MATCH (cours:dbjaap_Cours)-[r:COMMENCE_PAR]->(chapitre:dbjaap_Chapitre) WHERE cours.id = {cours_id} RETURN chapitre;', {
                cours_id: data
            })
                .then(function (result) {
    ...
    selectionIdCoursNeo4j(data) {
        return new Promise(function (resolve, reject) {
            let driver = neo.driver('bolt://' + host + ':7687', neo.auth.basic(username, passwd));
            let session = driver.session();
            session.run('MATCH (cours:dbjaap_Cours), (chapitre:dbjaap_Chapitre) WHERE chapitre.id={chapitre_id} MATCH (chapitre)-[a:APPARTIENT_A]-(cours) RETURN cours.id', {
                chapitre_id: data
            })
                .then(function (result) {
                if (result.records[0] == null) {
    ...
    selectionChapitreSuivantNeo4j(data) {
        return new Promise(function (resolve, reject) {
            let driver = neo.driver('bolt://' + host + ':7687', neo.auth.basic(username, passwd));
            let session = driver.session();
            session.run('MATCH (chapitre1:dbjaap_Chapitre)-[SUIVI_PAR]->(chapitre2:dbjaap_Chapitre) WHERE chapitre1.id={chapitre_id} RETURN chapitre2', {
    ...
    selectionChapitrePrecedantNeo4j(data) {
        return new Promise(function (resolve, reject) {
            let driver = neo.driver('bolt://' + host + ':7687', neo.auth.basic(username, passwd));
            let session = driver.session();
            session.run('MATCH (chapitre1:dbjaap_Chapitre)-[SUIVI_PAR]->(chapitre2:dbjaap_Chapitre) WHERE chapitre2.id={chapitre_id} RETURN chapitre1', {
                chapitre_id: JSON.stringify(data)
            })
                .then(function (result) {
                if (result.records[0] == null) {
